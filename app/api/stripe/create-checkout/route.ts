...
export async function POST() {
  ...
  if (!process.env.STRIPE_PRICE_ID_PREMIUM) {
    return NextResponse.json(
      { error: 'STRIPE_PRICE_ID_PREMIUM não configurada no servidor.' },
      { status: 503 },
    )
  }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : 'Erro desconhecido ao iniciar checkout'
    console.error('[create-checkout] falha:', msg)
